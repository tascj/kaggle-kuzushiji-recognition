...
test_cfg = dict(
    rpn=dict(
        ...
        min_bbox_size=0),
    rcnn=dict(
        score_thr=0.5, nms=dict(type='nms', iou_thr=0.5), max_per_img=700),
...
train_pipeline = [
    ...
    dict(
        type='Resize',
        img_scale=[(size, size) for size in range(512, 768 + 1, 32)],
        keep_ratio=True,
...
test_pipeline = [
    ...
    dict(
        type='MultiScaleAug',
        img_scale=[size / 1024 for size in range(512, 768 + 1, 128)],
        transforms=[
            dict(type='Resize', keep_ratio=True),
            # dict(type='RandomFlip'),
            dict(type='Normalize', **img_norm_cfg),
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        # ann_file=data_root + 'dtrain_crop.pkl',
